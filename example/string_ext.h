...
/* { */
/* 	elementType1* a1 = (elementType1*)a; */
/* 	elementType1* b1 = (elementType1*)b; */
/* 	return (a1.coef - b1.coef); */
/* } */



int partition(elementType1 *list, int first, int last)
{
	int leftMark = first +1;
	int rightMark = last;
	while(1)
	{
		while(leftMark <= rightMark && list[leftMark].exp <= list[first].exp)
			leftMark = leftMark + 1;
		while(leftMark <= rightMark && list[rightMark].exp >= list[first].exp)
			rightMark = rightMark -1;
		if(rightMark < leftMark)
			break;
		else {
			elementType1 temp = list[leftMark];
			list[leftMark] = list[rightMark];
			list[rightMark] = temp;
		}
	}

	elementType1 temp = list[first];
...
void quickSort(elementType1 *list, int first, int last)
{
	if(first < last)
	{
